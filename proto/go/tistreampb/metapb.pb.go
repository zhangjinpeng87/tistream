// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.28.1
// 	protoc        v3.8.0
// source: metapb.proto

package __

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type TaskOp int32

const (
	TaskOp_Attach TaskOp = 0
	TaskOp_Detach TaskOp = 1
)

// Enum value maps for TaskOp.
var (
	TaskOp_name = map[int32]string{
		0: "Attach",
		1: "Detach",
	}
	TaskOp_value = map[string]int32{
		"Attach": 0,
		"Detach": 1,
	}
)

func (x TaskOp) Enum() *TaskOp {
	p := new(TaskOp)
	*p = x
	return p
}

func (x TaskOp) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (TaskOp) Descriptor() protoreflect.EnumDescriptor {
	return file_metapb_proto_enumTypes[0].Descriptor()
}

func (TaskOp) Type() protoreflect.EnumType {
	return &file_metapb_proto_enumTypes[0]
}

func (x TaskOp) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use TaskOp.Descriptor instead.
func (TaskOp) EnumDescriptor() ([]byte, []int) {
	return file_metapb_proto_rawDescGZIP(), []int{0}
}

// Upstream notify downstream there are new data change files for specified tenants.
type UpdateNotificationReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	TenantId []uint32 `protobuf:"varint,1,rep,packed,name=tenant_id,json=tenantId,proto3" json:"tenant_id,omitempty"`
}

func (x *UpdateNotificationReq) Reset() {
	*x = UpdateNotificationReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_metapb_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateNotificationReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateNotificationReq) ProtoMessage() {}

func (x *UpdateNotificationReq) ProtoReflect() protoreflect.Message {
	mi := &file_metapb_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateNotificationReq.ProtoReflect.Descriptor instead.
func (*UpdateNotificationReq) Descriptor() ([]byte, []int) {
	return file_metapb_proto_rawDescGZIP(), []int{0}
}

func (x *UpdateNotificationReq) GetTenantId() []uint32 {
	if x != nil {
		return x.TenantId
	}
	return nil
}

type UpdateNotificationResp struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *UpdateNotificationResp) Reset() {
	*x = UpdateNotificationResp{}
	if protoimpl.UnsafeEnabled {
		mi := &file_metapb_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateNotificationResp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateNotificationResp) ProtoMessage() {}

func (x *UpdateNotificationResp) ProtoReflect() protoreflect.Message {
	mi := &file_metapb_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateNotificationResp.ProtoReflect.Descriptor instead.
func (*UpdateNotificationResp) Descriptor() ([]byte, []int) {
	return file_metapb_proto_rawDescGZIP(), []int{1}
}

type DispatcherHeartbeatReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	NodeId     uint32 `protobuf:"varint,1,opt,name=node_id,json=nodeId,proto3" json:"node_id,omitempty"` // dispatcher node id.
	Throughput uint64 `protobuf:"varint,2,opt,name=throughput,proto3" json:"throughput,omitempty"`       // throughput since last heartbeat.
}

func (x *DispatcherHeartbeatReq) Reset() {
	*x = DispatcherHeartbeatReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_metapb_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DispatcherHeartbeatReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DispatcherHeartbeatReq) ProtoMessage() {}

func (x *DispatcherHeartbeatReq) ProtoReflect() protoreflect.Message {
	mi := &file_metapb_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DispatcherHeartbeatReq.ProtoReflect.Descriptor instead.
func (*DispatcherHeartbeatReq) Descriptor() ([]byte, []int) {
	return file_metapb_proto_rawDescGZIP(), []int{2}
}

func (x *DispatcherHeartbeatReq) GetNodeId() uint32 {
	if x != nil {
		return x.NodeId
	}
	return 0
}

func (x *DispatcherHeartbeatReq) GetThroughput() uint64 {
	if x != nil {
		return x.Throughput
	}
	return 0
}

type DispatcherHeartbeatResp struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *DispatcherHeartbeatResp) Reset() {
	*x = DispatcherHeartbeatResp{}
	if protoimpl.UnsafeEnabled {
		mi := &file_metapb_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DispatcherHeartbeatResp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DispatcherHeartbeatResp) ProtoMessage() {}

func (x *DispatcherHeartbeatResp) ProtoReflect() protoreflect.Message {
	mi := &file_metapb_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DispatcherHeartbeatResp.ProtoReflect.Descriptor instead.
func (*DispatcherHeartbeatResp) Descriptor() ([]byte, []int) {
	return file_metapb_proto_rawDescGZIP(), []int{3}
}

type SorterHeartbeatReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	NodeId     uint32       `protobuf:"varint,1,opt,name=node_id,json=nodeId,proto3" json:"node_id,omitempty"` // sorter node id.
	Throughput uint64       `protobuf:"varint,2,opt,name=throughput,proto3" json:"throughput,omitempty"`       // throughput since last heartbeat.
	TaskStats  []*TaskStats `protobuf:"bytes,3,rep,name=task_stats,json=taskStats,proto3" json:"task_stats,omitempty"`
}

func (x *SorterHeartbeatReq) Reset() {
	*x = SorterHeartbeatReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_metapb_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *SorterHeartbeatReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SorterHeartbeatReq) ProtoMessage() {}

func (x *SorterHeartbeatReq) ProtoReflect() protoreflect.Message {
	mi := &file_metapb_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SorterHeartbeatReq.ProtoReflect.Descriptor instead.
func (*SorterHeartbeatReq) Descriptor() ([]byte, []int) {
	return file_metapb_proto_rawDescGZIP(), []int{4}
}

func (x *SorterHeartbeatReq) GetNodeId() uint32 {
	if x != nil {
		return x.NodeId
	}
	return 0
}

func (x *SorterHeartbeatReq) GetThroughput() uint64 {
	if x != nil {
		return x.Throughput
	}
	return 0
}

func (x *SorterHeartbeatReq) GetTaskStats() []*TaskStats {
	if x != nil {
		return x.TaskStats
	}
	return nil
}

type SorterHeartbeatResp struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *SorterHeartbeatResp) Reset() {
	*x = SorterHeartbeatResp{}
	if protoimpl.UnsafeEnabled {
		mi := &file_metapb_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *SorterHeartbeatResp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SorterHeartbeatResp) ProtoMessage() {}

func (x *SorterHeartbeatResp) ProtoReflect() protoreflect.Message {
	mi := &file_metapb_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SorterHeartbeatResp.ProtoReflect.Descriptor instead.
func (*SorterHeartbeatResp) Descriptor() ([]byte, []int) {
	return file_metapb_proto_rawDescGZIP(), []int{5}
}

type TenantStats struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id        uint32       `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"` // tenant id.
	TaskStats []*TaskStats `protobuf:"bytes,2,rep,name=task_stats,json=taskStats,proto3" json:"task_stats,omitempty"`
}

func (x *TenantStats) Reset() {
	*x = TenantStats{}
	if protoimpl.UnsafeEnabled {
		mi := &file_metapb_proto_msgTypes[6]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *TenantStats) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TenantStats) ProtoMessage() {}

func (x *TenantStats) ProtoReflect() protoreflect.Message {
	mi := &file_metapb_proto_msgTypes[6]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TenantStats.ProtoReflect.Descriptor instead.
func (*TenantStats) Descriptor() ([]byte, []int) {
	return file_metapb_proto_rawDescGZIP(), []int{6}
}

func (x *TenantStats) GetId() uint32 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *TenantStats) GetTaskStats() []*TaskStats {
	if x != nil {
		return x.TaskStats
	}
	return nil
}

type Task struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	TenantId   uint32 `protobuf:"varint,1,opt,name=tenant_id,json=tenantId,proto3" json:"tenant_id,omitempty"`
	RangeStart []byte `protobuf:"bytes,2,opt,name=range_start,json=rangeStart,proto3" json:"range_start,omitempty"`
	RangeEnd   []byte `protobuf:"bytes,3,opt,name=range_end,json=rangeEnd,proto3" json:"range_end,omitempty"`
	SorterId   uint32 `protobuf:"varint,4,opt,name=sorter_id,json=sorterId,proto3" json:"sorter_id,omitempty"`
	SnapAddr   string `protobuf:"bytes,5,opt,name=snap_addr,json=snapAddr,proto3" json:"snap_addr,omitempty"`
}

func (x *Task) Reset() {
	*x = Task{}
	if protoimpl.UnsafeEnabled {
		mi := &file_metapb_proto_msgTypes[7]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Task) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Task) ProtoMessage() {}

func (x *Task) ProtoReflect() protoreflect.Message {
	mi := &file_metapb_proto_msgTypes[7]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Task.ProtoReflect.Descriptor instead.
func (*Task) Descriptor() ([]byte, []int) {
	return file_metapb_proto_rawDescGZIP(), []int{7}
}

func (x *Task) GetTenantId() uint32 {
	if x != nil {
		return x.TenantId
	}
	return 0
}

func (x *Task) GetRangeStart() []byte {
	if x != nil {
		return x.RangeStart
	}
	return nil
}

func (x *Task) GetRangeEnd() []byte {
	if x != nil {
		return x.RangeEnd
	}
	return nil
}

func (x *Task) GetSorterId() uint32 {
	if x != nil {
		return x.SorterId
	}
	return 0
}

func (x *Task) GetSnapAddr() string {
	if x != nil {
		return x.SnapAddr
	}
	return ""
}

type TaskStats struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Task       *Task  `protobuf:"bytes,1,opt,name=task,proto3" json:"task,omitempty"`
	Throughput uint64 `protobuf:"varint,2,opt,name=throughput,proto3" json:"throughput,omitempty"` // throughput since last heartbeat.
}

func (x *TaskStats) Reset() {
	*x = TaskStats{}
	if protoimpl.UnsafeEnabled {
		mi := &file_metapb_proto_msgTypes[8]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *TaskStats) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TaskStats) ProtoMessage() {}

func (x *TaskStats) ProtoReflect() protoreflect.Message {
	mi := &file_metapb_proto_msgTypes[8]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TaskStats.ProtoReflect.Descriptor instead.
func (*TaskStats) Descriptor() ([]byte, []int) {
	return file_metapb_proto_rawDescGZIP(), []int{8}
}

func (x *TaskStats) GetTask() *Task {
	if x != nil {
		return x.Task
	}
	return nil
}

func (x *TaskStats) GetThroughput() uint64 {
	if x != nil {
		return x.Throughput
	}
	return 0
}

type TenantTasksReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	TenantId uint32  `protobuf:"varint,1,opt,name=tenant_id,json=tenantId,proto3" json:"tenant_id,omitempty"`
	Tasks    []*Task `protobuf:"bytes,2,rep,name=tasks,proto3" json:"tasks,omitempty"`
	Op       TaskOp  `protobuf:"varint,3,opt,name=op,proto3,enum=tistreampb.TaskOp" json:"op,omitempty"`
}

func (x *TenantTasksReq) Reset() {
	*x = TenantTasksReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_metapb_proto_msgTypes[9]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *TenantTasksReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TenantTasksReq) ProtoMessage() {}

func (x *TenantTasksReq) ProtoReflect() protoreflect.Message {
	mi := &file_metapb_proto_msgTypes[9]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TenantTasksReq.ProtoReflect.Descriptor instead.
func (*TenantTasksReq) Descriptor() ([]byte, []int) {
	return file_metapb_proto_rawDescGZIP(), []int{9}
}

func (x *TenantTasksReq) GetTenantId() uint32 {
	if x != nil {
		return x.TenantId
	}
	return 0
}

func (x *TenantTasksReq) GetTasks() []*Task {
	if x != nil {
		return x.Tasks
	}
	return nil
}

func (x *TenantTasksReq) GetOp() TaskOp {
	if x != nil {
		return x.Op
	}
	return TaskOp_Attach
}

type TenantTasksResp struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *TenantTasksResp) Reset() {
	*x = TenantTasksResp{}
	if protoimpl.UnsafeEnabled {
		mi := &file_metapb_proto_msgTypes[10]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *TenantTasksResp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TenantTasksResp) ProtoMessage() {}

func (x *TenantTasksResp) ProtoReflect() protoreflect.Message {
	mi := &file_metapb_proto_msgTypes[10]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TenantTasksResp.ProtoReflect.Descriptor instead.
func (*TenantTasksResp) Descriptor() ([]byte, []int) {
	return file_metapb_proto_rawDescGZIP(), []int{10}
}

var File_metapb_proto protoreflect.FileDescriptor

var file_metapb_proto_rawDesc = []byte{
	,
}

var (
	file_metapb_proto_rawDescOnce sync.Once
	file_metapb_proto_rawDescData = file_metapb_proto_rawDesc
)

func file_metapb_proto_rawDescGZIP() []byte {
	file_metapb_proto_rawDescOnce.Do(func() {
		file_metapb_proto_rawDescData = protoimpl.X.CompressGZIP(file_metapb_proto_rawDescData)
	})
	return file_metapb_proto_rawDescData
}

var file_metapb_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_metapb_proto_msgTypes = make([]protoimpl.MessageInfo, 11)
var file_metapb_proto_goTypes = []interface{}{
	(TaskOp)(0),                     // 0: tistreampb.TaskOp
	(*UpdateNotificationReq)(nil),   // 1: tistreampb.UpdateNotificationReq
	(*UpdateNotificationResp)(nil),  // 2: tistreampb.UpdateNotificationResp
	(*DispatcherHeartbeatReq)(nil),  // 3: tistreampb.DispatcherHeartbeatReq
	(*DispatcherHeartbeatResp)(nil), // 4: tistreampb.DispatcherHeartbeatResp
	(*SorterHeartbeatReq)(nil),      // 5: tistreampb.SorterHeartbeatReq
	(*SorterHeartbeatResp)(nil),     // 6: tistreampb.SorterHeartbeatResp
	(*TenantStats)(nil),             // 7: tistreampb.TenantStats
	(*Task)(nil),                    // 8: tistreampb.Task
	(*TaskStats)(nil),               // 9: tistreampb.TaskStats
	(*TenantTasksReq)(nil),          // 10: tistreampb.TenantTasksReq
	(*TenantTasksResp)(nil),         // 11: tistreampb.TenantTasksResp
}
var file_metapb_proto_depIdxs = []int32{
	9, // 0: tistreampb.SorterHeartbeatReq.task_stats:type_name -> tistreampb.TaskStats
	9, // 1: tistreampb.TenantStats.task_stats:type_name -> tistreampb.TaskStats
	8, // 2: tistreampb.TaskStats.task:type_name -> tistreampb.Task
	8, // 3: tistreampb.TenantTasksReq.tasks:type_name -> tistreampb.Task
	0, // 4: tistreampb.TenantTasksReq.op:type_name -> tistreampb.TaskOp
	5, // [5:5] is the sub-list for method output_type
	5, // [5:5] is the sub-list for method input_type
	5, // [5:5] is the sub-list for extension type_name
	5, // [5:5] is the sub-list for extension extendee
	0, // [0:5] is the sub-list for field type_name
}

func init() { file_metapb_proto_init() }
func file_metapb_proto_init() {
	if File_metapb_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_metapb_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateNotificationReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_metapb_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateNotificationResp); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_metapb_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DispatcherHeartbeatReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_metapb_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DispatcherHeartbeatResp); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_metapb_proto_msgTypes[4].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*SorterHeartbeatReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_metapb_proto_msgTypes[5].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*SorterHeartbeatResp); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_metapb_proto_msgTypes[6].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*TenantStats); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_metapb_proto_msgTypes[7].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Task); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_metapb_proto_msgTypes[8].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*TaskStats); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_metapb_proto_msgTypes[9].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*TenantTasksReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_metapb_proto_msgTypes[10].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*TenantTasksResp); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_metapb_proto_rawDesc,
			NumEnums:      1,
			NumMessages:   11,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_metapb_proto_goTypes,
		DependencyIndexes: file_metapb_proto_depIdxs,
		EnumInfos:         file_metapb_proto_enumTypes,
		MessageInfos:      file_metapb_proto_msgTypes,
	}.Build()
	File_metapb_proto = out.File
	file_metapb_proto_rawDesc = nil
	file_metapb_proto_goTypes = nil
	file_metapb_proto_depIdxs = nil
}
